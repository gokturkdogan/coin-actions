...
const actions = {
  ...
  startAggTradeSocket({ commit, state }) {
    ...
    const ws = new WebSocket(`wss://fstream.binance.com/stream?streams=${streams}`); // ← VADİLİ (Futures) WebSocket
    ...
    ws.onopen = () => {
      console.log('✅ Futures aggTrades websocket bağlantısı açıldı');
    };
    ...
    ws.onclose = () => {
      console.log('❌ Futures aggTrades websocket kapandı');
      commit('clearAggTradeSocket');
    };
